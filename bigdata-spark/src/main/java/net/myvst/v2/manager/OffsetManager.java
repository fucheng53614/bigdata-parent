...
@Slf4j
public class OffsetManager implements OffsetManagerInterface {
    ...
    public void store(DBOperator db, Object obj) throws SQLException {
        ...
        Object[][] objects = new Object[offsetRanges.length][];
        for (int i = 0; i < offsetRanges.length; i++) {
            ...
            objects[i] = new Object[]{id, topic, partition, fromOffset, untilOffset, fromOffset, untilOffset};
        }
        String insert = "UPSERT INTO " + getTableName() + "(id, topic, partition, fromOffset, untilOffset) VALUES(?,?,?,?,?) " +
                "ON DUPLICATE KEY UPDATE fromOffset=?,untilOffset=?";

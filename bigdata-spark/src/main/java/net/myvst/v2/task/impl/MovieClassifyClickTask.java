...
public class MovieClassifyClickTask implements Task {
    ...
    public Object process(JavaRDD<String> rdd) {
        JavaPairRDD<String, StatCounter> pairRDD = rdd.map(JSONObject::parseObject)
                ...
                .mapToPair(json -> {
                    ...
                    String key = pkg + "[@!@]" + date + "[@!@]" + cid + "[@!@]" + name + "[@!@]" + nameId + "[@!@]";
    ...
    public void store(DBOperator db, Object obj) throws SQLException {
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            objects[i] = new Object[]{md5Id, idSplit[0], idSplit[1], idSplit[2], idSplit[3], uv, vv, curTime, curTime, uv, vv, curTime};
        }
